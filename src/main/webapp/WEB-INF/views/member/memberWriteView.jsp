...
<body>
    ...
    <div id="cover" class="container-fluid">
	    <div>
	        <h1>회원가입</h1>
	        <p>회원가입 후에 근무지  및 프로젝트 팀을 조회 가능하고<br />
...
<form action="memberWrite" method="post" class="container" onsubmit="return writeSave()">
			...
			<table class="table table-striped" width="500" cellpadding="0" cellspacing="0" border="1">
				...
				<tr>
					<th>근무지</th>
					<td>
					<!-- <select name="mloc" size="1">
				                <option value="수원">수원</option>
				                <option value="정자">정자</option>
      				</select> -->
      				 <input type="checkbox" name="mloc" value="수원" onclick="doOpenCheck(this);">수원<br>
 					 <input type="checkbox" name="mloc" value="정자" checked="checked" onclick="doOpenCheck(this);">정자<br>
      				</td>
				</tr>
...
function doOpenCheck(chk){
    var obj = document.getElementsByName("mloc");
...
$(document).ready(function() {
	 ...
	 $('.container').on('submit',function(event){
	   var idCheckBtn =  $("#idCheck").val();
		 if(idCheckBtn != "idCheckOk"){
		 	
			event.preventDefault() ;
	       	event.stopPropagation();
	        swal("아이디 중복검사를 확인해주세요.");
	 	 
		 }
	 });
	
	//아이디 중복확인 버튼 클릭시 이벤트 발생
